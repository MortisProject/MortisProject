...
namespace Player.Animation
{
    ...
    /// 모든 상태(State)는 이 스크립트를 통해 애니메이션 재생을 요청
    /// </summary>
    [RequireComponent(typeof(Animator))]
    public class PlayerAnimationController : MonoBehaviour
    {
        [Header("Settings")]
        [Tooltip("애니메이션 파라미터가 목표 값으로 변하는 데 걸리는 시간")]
        ...
        /// <summary>
        /// 매 프레임 호출되어 현재 애니메이션 값을 목표 값으로 부드럽게 이동
        /// </summary>
        private void Update()
        {
            // 현재 애니메이터의 MoveX, MoveY 값을 가져온다
            float currentX = _animator.GetFloat(_moveXHash);
            float currentY = _animator.GetFloat(_moveYHash);

            
            // Mathf.SmoothDamp를 사용하여 현재 값을 목표 값으로 부드럽게 보간
        ...
        /// <summary>
        /// 이동 관련 애니메이터 파라미터와 내부 변수들을 즉시 0으로 초기화
        /// SmoothDamp를 무시하고 값을 강제로 리셋
        /// </summary>
        public void ResetMoveParameters()
        {
            ...
            // 2. SmoothDamp가 사용하는 현재 속도 값도 0으로 초기화
            _currentMoveVelocity = Vector2.zero;

            // 3. 애니메이터의 파라미터 값을 즉시 0으로 강제 설정
