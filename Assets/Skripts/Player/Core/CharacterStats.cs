...
namespace Player
{
    ...
    public class CharacterStats : MonoBehaviour
    {
        ...
        [Header("Abilities")]
        [Tooltip("현재 더블 점프가 가능한지 여부")]
        public bool CanDoubleJump { get; private set; }

        [Header("Weapon System")]
        [Tooltip("플레이어가 사용할 수 있는 모든 무기 데이터 목록")]
        public List<WeaponData> availableWeapons;

        [Tooltip("실제 무기 모델 GameObject 리스트, availableWeapons 리스트와 순서 및 개수가 반드시 일치해야함")]
        ...
        /// <summary>
        /// 지정된 양의 아스트를 소모 (성공: true, 실패: false)
        /// </summary>
        ...
        /// <summary>
        /// 지정된 양의 버스트 게이지를 소모 (성공: true, 실패: false)
        /// </summary>
        ...
        /// <summary>
        /// 지정된 양의 데미지를 받고, 공격 타입에 따라 추가 처리
        /// </summary>
        //넉백타입이 제거되지 않은 코드
        ...
        public void TakeDamage(float damage, Transform attacker, MonsterSkillData.AttackType attackType)
        {
            ...
            if (currentState is PlayerGuardState guardState)
            {
                ...
                // 데미지를 받고 넉백을 적용한 후 GuardBreakState로 전환
                currentHp -= damage;
                OnHpChanged?.Invoke(currentHp, maxHp);
                CameraManager.Instance.PlayImpulse(CameraManager.ImpulseShakeType.WeakHit);
                VFXManager.Instance.PlayVFX("PlayerHit", HitEffectTarget.position);
        ...
        /// <summary>
        /// 버스트 게이지가 변경될 때 UI 등에 알리기 위한 이벤트 (현재값, 최대값)
        /// </summary>
        public event Action<float, float> OnBustChanged;
        /// <summary>
        /// 체력이 변경될 때 UI 등에 알리기 위한 이벤트 (현재값, 최대값)
        /// </summary>
        public event Action<float, float> OnHpChanged;

        /// <summary>
        /// 아스트가 변경될 때 UI 등에 알리기 위한 이벤트 (현재값, 최대값)
        /// </summary>
        public event Action<float, float> OnAstChanged;

        /// <summary>
        /// 무기가 교체될 때 UI 등에 알리기 위한 이벤트 (새 주무기 데이터, 새 보조무기 데이터)
        /// </summary>
